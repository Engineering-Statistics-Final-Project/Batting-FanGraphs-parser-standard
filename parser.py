...
for month in range(1995,2020):
    url_string = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=bat&lg=all&qual=0&type=0&season="
    url_month  = str(month)
    url_string2 = "&month=0&season1="
    url_string3 = "&ind=0&team=0,ts&rost=0&age=14,58&filter=&players=0&startdate=2020-01-01&enddate=2020-12-31"
...
for month2 in range(1995,2020):
    ...
    if(month2>1997):
        ...
        for i in range(0,30):
            for j in range(1,22):
                if(j==1): FILE_ans.write('{:15}'.format('"'+ans[i*23+j]+'"'))
                else: FILE_ans.write('{:8}'.format('"'+ans[i*23+j]+'"'))
            FILE_ans.write("\n")
